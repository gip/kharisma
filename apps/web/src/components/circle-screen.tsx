...
export function CircleScreen({ groupId }: { groupId: string }) {
  ...
  const visibleSenderInboxIds = group
    ...
    : [];
  const visibleSenderInboxIdsKey = visibleSenderInboxIds.join(",");
  const threadListOptions =
    messageVisibility === "human" ? { visibleSenderInboxIds } : undefined;
  ...
  }, [canLoadThreads, groupId, messageVisibility, visibleSenderInboxIdsKey]);
      ...
      {group ? (
        <InvestModal
          ...
          onRecorded={() => {
            ...
            if (canLoadThreads) {
              void listGroupThreads(groupId, threadListOptions)
                .then((next) => setThreads(next))
                .catch(() => undefined);
            }
          }}
